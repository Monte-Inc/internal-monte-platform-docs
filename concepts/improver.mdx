...
Two verbs sit beside the Move. The Improver nominates the Checkpoint its Envelope presents, and it can nominate only one that already has an admissible `dev` eval, because a presented model is measured rather than liked. Re-nominating replaces the standing Nomination, and an Envelope whose Improver never nominated falls back to the claim rule instead. The Improver can also conclude, which ends the Envelope early on the conviction that more Moves will not improve the presented result, in either direction. The finalizer then runs at once rather than at the window's close, and the window stays the hard ceiling.
...
After the window closes, or as soon as the Improver concludes, a finalizer runs a fixed sequence on the box. Two of its steps act on the presented Checkpoint, which is the standing Nomination when the Improver left one, and otherwise the Checkpoint with the highest paired `dev` delta over the Baseline, with ties going to the higher n and then to the newer Run.
