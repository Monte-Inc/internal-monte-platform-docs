...
The other pages here call the thing that works the loop a driving agent. The Improver is Monte's own: an outer-loop agent that reads a Measurement's Ledger, decides the next [Move](/concepts/improvement), runs it, and reads what came back. It runs unattended for a fixed stretch of time on a GPU box. That makes the interesting question not how it decides, but what it is allowed to do while nobody is watching. The answer is the Envelope, and the platform enforces it rather than the prompt.

## One turn of the loop

The Improver works one Measurement at a time, and a turn of its loop is four steps.

1. **Read the Ledger.** `monte status` prints the per-Cohort deltas against the Baseline, the Cycles with their strike verdicts, and the Checkpoint tree with a `<run>/<step>` name for every Checkpoint in it.
...
- **`monte eval --split test` is refused**, in every spelling, `--smoke` included. A smoke Run skips the two-read budget on `test`, so it is the one spelling that would otherwise work.
...
4. **Push the claimed Checkpoint's weights** to the durable tier.
5. **Sync**, which carries the results tree and the Improver's own transcript.
6. **Release the box to idle**, rather than terminating it, since the box outlives the Envelope.
