...
| Term | What it states |
| --- | --- |
| **Measurement** | The Measurement the search improves. Its `dev` Split is the Improver's only score. |
...
- **`monte eval <measurement> --split test` is refused**, in every spelling, `--smoke` included. A smoke Run skips the two-read budget on `test`, so it is the one spelling that would otherwise work.
...
Each of those is keyed to one Envelope's id, and an Envelope binds only the Measurement it names on the box it runs on. There is no global mode anywhere in this. A person evaluating a different Measurement is never refused by it, and neither is a second search on a second box.
...
After the window closes, or as soon as the Improver concludes, a finalizer runs a fixed sequence on the box. Two of its steps act on the presented Checkpoint. That is the standing Nomination when the Improver left one, and otherwise the Checkpoint with the highest paired `dev` delta over the Baseline, with ties going to the higher n and then to the newer Run.
...
3. **Make the blind claim**, on closing Envelopes only, as an eval on `test` of the presented Checkpoint. When the Nomination is not the Checkpoint the rule would have picked, the finalizer scores the rule's pick on `test` beside it. That counterfactual read is diagnostic: it selects nothing, pushes no weights, and is the one sanctioned addition to the Split's two reads.
...
`list` prints every Envelope record this host can see, each with its state, its claim bit, its Measurement, and its window. `show` prints one in full: the approved terms exactly as they were approved, plus the state Monte keeps alongside them.
