...
| **Vault** | The cloud bucket holding what `monte env add` and `monte dataset add` froze. It is the copy Monte treats as true, and a data root holds a materialization of it. `monte env install` fetches an Environment back, and `monte dataset show` fetches a Dataset. An Environment shipped as a package never enters it. See [Environments](/concepts/environments#where-an-environment-lives). |
