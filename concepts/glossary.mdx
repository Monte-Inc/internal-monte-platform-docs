...
| **Vault** | The cloud bucket holding every frozen Environment and Dataset. It is the copy Monte treats as true, and a data root holds a materialization of it. `monte env install` fetches an Environment back, and a Dataset is fetched on read. See [Environments](/concepts/environments#where-an-environment-lives). |
