...
| **Mirror** | The read-only local copy of a Measurement's results that `monte sync` pulls from the results bucket, so Ledger state reads with no box up. Weights never enter one, and neither do frozen inputs. |
