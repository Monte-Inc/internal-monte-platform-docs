...
Checkpoints therefore form a tree rather than a line: two Runs may start from the same Checkpoint and go different ways, and a Run may start from any Checkpoint in its Cohort. A Cohort is a set (what is comparable); a Lineage is a path (what came from what). `monte status` prints one line per Lineage inside each Cohort, and `--json` carries the same paths with their Recipes and per-hop evals.
