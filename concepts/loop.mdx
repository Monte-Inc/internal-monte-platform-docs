...
Two loops are running. Training moves weights on the `train` Split, and that is the inner loop. The agent then reads the dev score from the Ledger, changes the Recipe or the hyperparameters, and trains again. That is the outer loop.

```mermaid actions={false}
flowchart TD
    subgraph INNER["Inner loop: moves weights"]
        TRAIN["train Split"] --> RUN["Training Run"] --> CKPT["Checkpoint"]
    end
    subgraph OUTER["Outer loop: moves the method"]
        EVAL["Eval on dev"] --> LEDGER["Ledger"] --> AGENT["Agent picks the next<br/>Recipe and hyperparameters"]
    end
    CKPT --> EVAL
    AGENT --> RUN
    TEST["test Split<br/>outside both loops<br/>Baseline and final claim only"]
```

The outer loop optimizes against dev, which is why dev cannot be the number you report.
