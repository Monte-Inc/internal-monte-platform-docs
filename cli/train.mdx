...
**`--source` is what this Run trains on.** It names either a trainable Environment's curriculum, or a frozen [Dataset](/cli/env#monte-dataset). This experiment's own Environment is a source like any other, and you state it like any other. The namespace is every trainable Environment plus every frozen Dataset on the data root. One name claimed by both refuses.
