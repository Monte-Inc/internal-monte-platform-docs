...
`add` checks for that key before it runs prepare, because prepare downloads from live sources and a 403 found later throws the whole freeze away. No writable key is not a refusal. The freeze happens on this data root alone, and the note says so and names the key. Freeze again as soon as you have it. Nothing is bound to a new Environment yet, so a re-freeze costs nothing today. Once a Baseline exists it is a different Measurement.

A push that fails after the freeze is the other case: a name that lost its race to another region, or a dropped connection. Monte removes the manifest it just wrote, so nothing discovers the directory and the add re-runs. A lost race names both halves of its fix: re-add under `--as <new name>`, and `monte env install <name>` brings the winner's copy here.
...
| `MONTE_GCS_KEY_FILE` | GCS service-account JSON path, and the ladder every read resolves: `sync`, `env install`, `dataset show`, and both `--vault` listings. On a laptop this is the read-only key. On a box it is the read-write one, planted there by a `--remote` submission | unset |
| `MONTE_GCS_KEY_OP` | 1Password document holding it instead | unset |
| `MONTE_GCS_BOX_KEY_FILE` | the read-write key. Every write resolves it first: `push`, `--push`, and a Vault freeze. The two ladders are separate, so a laptop that both reads and freezes sets one variable from each pair | unset |
| `MONTE_GCS_BOX_KEY_OP` | 1Password document holding that one. The default is used when `--remote` plants a key on a box, and nowhere else, so a write from your own machine needs one of these two set | `GCS SA key - monte-box (rw monte-platform-results)` |
