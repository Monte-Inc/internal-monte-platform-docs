...
`--recipe` names the [Recipe](/concepts/training#recipes) for this Run, defaulting to the Measurement's. A Recipe may name its own model, and `--model` still wins. Both the name and the file's content hash land in the Run's plan, and the bytes in the run directory.
