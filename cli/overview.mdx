...
| `MONTE_GCS_KEY_FILE` | the key every read resolves, before `MONTE_GCS_KEY_OP` and nothing else: `sync`, `env install`, `dataset show`, both `--vault` listings. A laptop's cannot write either bucket. A box's can, because `--remote` plants the read-write key here | unset |
| `MONTE_GCS_KEY_OP` | 1Password document holding it instead | unset |
| `MONTE_GCS_BOX_KEY_FILE` | the read-write key, tried first by every write: `push`, `--push`, and a Vault freeze. A write falls back to the read pair above; a read never reaches for this one | unset |
| `MONTE_GCS_BOX_KEY_OP` | 1Password document holding that one. The default applies when `--remote` plants a key on a box and nowhere else, so writing from a laptop means setting one of these two yourself | `GCS SA key - monte-box (rw monte-platform-results)` |
...
| 9 | `E_NOT_FOUND` | no such Measurement, Run, Environment, Dataset, or Checkpoint. Also a Vault fetch with no key to read with, which cannot tell the two apart |
...
| 11 | `E_CORRUPT_STATE` | Ledger or pairing state is inconsistent, a frozen Dataset's rows fail their pin, or bytes pulled from the Vault fail theirs. A frozen Environment already on the data root refuses under code 3 instead |
