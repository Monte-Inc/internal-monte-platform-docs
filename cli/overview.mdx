...
A Dataset is a frozen pile of rows, and it is what an `sft` Recipe trains on. `add` fetches the rows, proves every line is a JSON object, records a hash of the file, and records where the rows came from. It then pushes the frozen rows to the Vault, under the same key rule as `env add`. Frozen means frozen, so re-adding a name refuses. A name claimed in the Vault refuses from any box in any region, and it refuses across kinds: an Environment in the Vault takes that name from a Dataset, and the reverse. Locally the two only collide when the Environment is trainable, so the Vault's rule is the stricter one.
...
| `MONTE_GCS_KEY_FILE` | GCS service-account JSON path. The read ladder: `sync`, `env install`, and both `--vault` listings. The laptop key belongs here, and it cannot write either bucket | unset |
