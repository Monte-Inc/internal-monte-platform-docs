...
- **Default subcommand.** `monte eval math` means `monte eval run math`. The same applies to `train`. Options may precede the name: `monte eval --smoke math` works. The `experiment`, `env`, `dataset`, `box`, and `weights` groups have no default on purpose. Their subcommands are different intents.
...
- **`--json`.** Supported on `eval run`, `train run`, `status`, `show`, `config`, `sync`, `push`, `env list`, `env add`, every `dataset` subcommand, every `box` subcommand, and `weights push`. Stdout carries pure JSON, and the human plan summary moves to stderr. One trap: `status --json` omits the checkpoint list that the human view prints under `checkpoints:`.
...
| Box-side | the GPU box holding the Checkpoint | real `eval` and `train`, `--push`, `promote`, `weights push` |
