...
| 17 | `E_ENVELOPE` | the action is outside an Envelope's approved terms: `eval --split test` or `promote` while one is active, a Run started past its window, or a box launch the Envelope does not cover |

An Envelope, in code 17, is the approved terms of one automated search: the window it runs in, what it may spend, and which box and Measurement it covers. Only a host running one ever sees that code.
