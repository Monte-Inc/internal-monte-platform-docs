...
That push resolves the read-write key: `MONTE_GCS_BOX_KEY_FILE`, then `MONTE_GCS_BOX_KEY_OP`, then the ordinary ladder. Resolve the read-only one and the upload refuses, and the refusal names those two variables. That refusal lands after the local freeze, and it takes the manifest with it: the frozen files stay on the data root, nothing discovers them as an Environment, and the add re-runs under the same name once the key is right. With no GCS key at all the add keeps the manifest, freezes on this data root alone, and prints a note that nothing durable holds it. An Environment in that state is one you keep yourself.
...
A Dataset is a frozen pile of rows, and it is what an `sft` Recipe trains on. `add` fetches the rows, proves every line is a JSON object, records a hash of the file, and records where the rows came from. It then pushes the frozen rows to the Vault, under the same key rule as `env add`. Frozen means frozen, so re-adding a name refuses. A name claimed in the Vault refuses from any box in any region.

A trainable Environment's curriculum and a Dataset share the `--source` namespace, so the two kinds guard each other's names. `dataset add` refuses a name the Vault already holds an Environment under. `env add --server` refuses a name a Dataset holds. The Dataset side is the stricter one, because a Vault listing does not say whether an Environment is trainable, so it refuses on the name alone. An eval-only `env add` checks nothing, because its name never resolves as a source.
...
A frozen Dataset lives in [the Vault](/concepts/environments#where-an-environment-lives) under the same rules an Environment does. The copy on the data root is a materialization of it, and `show` is the command that fetches that copy back. Nothing else does: `monte train --source <name>` resolves against the data root alone, so run `monte dataset show <name>` first on a machine that has never held the rows. That fetch needs a GCS key. Without one `show` refuses as `E_NOT_FOUND`, which reads like a Dataset that does not exist.
...
Both take `--key-file` first. After that they differ, because one reads and one writes. `sync` resolves `MONTE_GCS_KEY_FILE`, then the 1Password document `MONTE_GCS_KEY_OP` names. `push` resolves the read-write key first, `MONTE_GCS_BOX_KEY_FILE` then `MONTE_GCS_BOX_KEY_OP`, and falls back to `sync`'s ladder. Every write goes that way, a Vault freeze included. Keys are fetched in memory. Without one, `E_STORAGE`.
...
| `MONTE_GCS_KEY_FILE` | GCS service-account JSON path, and the ladder every read resolves: `sync`, `env install`, and both `--vault` listings. On a laptop this is the read-only key. On a box it is the read-write one, planted there by `box up` | unset |
| `MONTE_GCS_KEY_OP` | 1Password document holding it instead | unset |
| `MONTE_GCS_BOX_KEY_FILE` | the read-write key. Every write resolves it first: `push`, `--push`, and a Vault freeze. Also the key `box up` plants on a box | unset |
| `MONTE_GCS_BOX_KEY_OP` | 1Password document holding that one. The default applies to `box up` alone, so a write from your own machine needs one of these two set | `GCS SA key - monte-box (rw monte-platform-results)` |
...
| 11 | `E_CORRUPT_STATE` | Ledger or pairing state is inconsistent, or a frozen file is missing, unreadable, or does not match its pin |
