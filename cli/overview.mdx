...
Monte chooses that Checkpoint, not you. Where the search left a Nomination, the finalizer claims and pushes that Checkpoint. Where it left none, Monte falls back to a rule. Among the admissible dev evals in the Cohort that scored a Checkpoint, the rule takes the one whose dev score improved most over the Baseline. Ties go to the eval at the higher step, then to the newer Run. The fallback refuses unless every one of those candidates is pairable, meaning every candidate's per-task results file can be read, not just the leader's. Taking the best of the readable ones would be a guess, because the real winner could be the candidate that could not be read. The fix is to run the command where the run directories live, which is the box.
...
**What a strike is.** Each Cycle carries a verdict. `STRIKE` means the dev delta against the Baseline is not significantly positive. `ok` means it is. A regression large enough to be significant is also a strike, because it is not an improvement either. A Cycle whose per-task files cannot be read prints neither verdict: it exists, but it is unjudged. Strikes are labels you read, not a countdown. Nothing adds them up, and no number of them ends anything. The loop ends at the Envelope's window close, when the Improver concludes, or when you stop it yourself.
