...
That push needs a GCS key that can write. The laptop key reads the Vault and cannot write it, so a freeze belongs on a box. The refusal says so, and it lands after the local freeze: the files are on this data root, the Vault holds nothing, and the name is still free there. With no GCS key at all the add freezes the same way and prints a note that nothing durable holds it. An Environment in that state is one you keep yourself.
