...
`--remote` submits the Run to an ephemeral GPU box and returns. `--push` uploads results to the results bucket, the cloud store that outlives any box, after each transition. It is what the box job itself runs with. See [Remote runs](#remote-runs).
...
`add` freezes what the Gym checkout ships into an Environment of your own. It runs Gym's prepare once, records a hash of the files, writes the manifest, and pushes the frozen files to [the vault](/concepts/environments#where-an-environment-lives). That prepare is the only step that reads a live source: every step after it moves bytes that are already pinned, and checks them against those hashes. Frozen means frozen, so re-adding a name refuses. The vault holds the name, so it refuses from any box in any region.
...
`list` has three scopes, and they read as one progression.
...
An Environment moves down that table through its life: it ships in Gym's catalog, `env add` freezes it into the vault, and `env install` makes it usable here. Bare `list` prints each Environment, its kind, and its Gym benchmarks, and it stays local, instant, and safe with no network. That is why `--vault` is a flag: the same discovery path runs on every `monte` command, so the default must never reach the network.

`install` makes a frozen Environment usable on this machine. When the data root does not hold the files, `install` fetches them from the vault and proves every hash before it writes them. When the data root does hold them, it verifies them where they are and fetches nothing. It then registers the benchmark configs. Install is idempotent. A mock-only Environment declares no Gym benchmarks, so it has nothing to register: `install` prints a note instead of installing anything, and that is success rather than a failure.
...
A Dataset is a frozen pile of rows, and it is what an `sft` Recipe trains on. `add` fetches the rows, proves every line is a JSON object, records a hash of the file, records where the rows came from, and pushes the frozen rows to the vault. Frozen means frozen, so re-adding a name refuses from any box in any region.
...
A frozen Dataset lives in the vault, the same as an Environment. The copy on the data root is a materialization of it: a read that does not find it there fetches it from the vault and proves the hash first.
...
`sync` and `push` are the two directions of the results bucket, the cloud store that outlives any box. It holds results only. Frozen Environments and Datasets live in the other bucket, [the vault](/concepts/environments#where-an-environment-lives), and `monte env` and `monte dataset` are what move those.
...
2. **A box.** Monte reuses an active box when it fits. If none fits, Monte launches and arms one. A real Run needs a warm box, because the Measurement it reads and the rows it writes live on the data root.
...
| `MONTE_GCS_KEY_FILE` | GCS service-account JSON path for `sync`, `push`, and the vault | unset |
...
| 14 | `E_STORAGE` | a cloud bucket is unreachable: credentials, package, or path |
