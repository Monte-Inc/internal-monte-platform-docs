...
- **Vault** (2026-08-18, ADR 0006) — the cloud bucket that `monte env add` and `monte dataset add` push what they freeze to. Capitalize it, as `CONTEXT.md` does. It is the source of truth for what reached it, and the copy under a data root is a **materialization** — never a Mirror, which means results only. Three things stay out of it: an add on a machine with no GCS key, which freezes locally and pushes nothing; an add whose key resolves and cannot write, which refuses and removes the manifest it wrote, so nothing is frozen at all; and an Environment shipped as a package, which is never pushed. So "every frozen input is in the Vault" is not a claim to make, and the three are not interchangeable. There are two cloud buckets now, so name the one you mean: write "the Vault" or "the results bucket". The old singular, "the durable tier", named one bucket back when there was one and no longer says which. Applied across this site 2026-08-18.
