...
- **Run detail**: the row in full, across tabs. Overview carries the plan settings, base model, serving stack, checkpoint relations, and live events. A training Run also gets a **Metrics** tab holding every per-step metric the trainer recorded.
...
- Metrics appear only after a training Run finishes and its checkpoint is assembled. A Run still training shows none.
- There is no archive or delete.
